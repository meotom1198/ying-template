...
$('.loadmore').click(function(){
    ...
    　　　　 $.ajax({
                ...
                error: function(data) {console.log('Yêu cầu tải cuộn không thành công!');},
            });
        }else {
            $(".loadmore").text("Không còn gì nữa~~")
        }
});
...
$(document).ready(function() {
    ...
    $('#post-pass').on('click', function() {
        ...
        $.ajax({
            ...
            cache: false,
            success: function(data) {
                if(data.indexOf("Sai mật khẩu") >= 0 && data.indexOf("<title>Error</title>") >= 0){
                    alert("Lỗi mật khẩu, vui lòng thử lại!");
                }else{
...
$(document).ready(function() {
    if ($("#chart-1").hasClass("gdtu")) {
        ...
        var option = {
            baseOption: {
                title: {
                    text: 'Thống kê xuất bản bài viết',
                    textStyle: {
                ...
                series: [{
                    ...
                    name: 'Số lượng bài viết đã đăng',
...
$(document).ready(function() {
    ...
    //Lặp qua từng khối mã trước và thêm
    
    for (var i = 0; i < codeblocks.length; i++) {
        //Hiển thị nút sao chép mã
        ...
        //Ẩn tất cả các nút "Sao chép"
        copy.style.visibility = "display"
    }
    ...
    for (var i = 0; i < codeblocks.length; i++) {
        ...
        !function (i) {
            //Khi chuột di chuyển qua khối mã, nút được hiển thị
            ...
            function copyArticle(event) {
                ...
                setTimeout(function () {
                    codeblocks[i].childNodes[1].innerHTML = "<i class='fa fa-clone'></i>"
                }, 1000);
                //Lựa chọn rõ ràng
        ...
        !function (i) {
            ...
            codeblocks[i].onmouseout = function () {
                codeblocks[i].childNodes[1].style.visibility = "display"
            }
        }(i);
    };
